...
## Redpanda Cloud-platform feature compatibility

Redpanda Cloud does not support the following platform (self-hosted) functionality:

- mTLS
- Remote Read Replicas (including Azure Blob Storage)
...
- The following `rpk` commands (which use the Admin API):
  ...
  - `rpk generate app`
  - `rpk redpanda`
  - `rpk topic describe-storage` (all other `rpk topic` commands are supported on both Redpanda platform and Redpanda Cloud)
