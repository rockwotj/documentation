...
- After calling DeleteRecords, local storage and object storage may contain data from before the offset. (Redpanda does not truncate segments in half. It deletes as many segments as possible, then bumps the start offset.) Data before the new start offset is not visible to clients but could be read by someone with access to the local disk of a Redpanda node. 
:::