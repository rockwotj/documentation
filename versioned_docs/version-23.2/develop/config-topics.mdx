...
- Object storage is deleted asynchronously. When DeleteRecords returns, the partition's start offset will have advanced, but the work required to garbage collect delete segments may not be complete.
- After calling DeleteRecords, local storage and object storage may contain data from before the offset. (Redpanda does not truncate segments in half. It deletes as many segments as possible for the last segment, then bumps the start offset.) This data is not visible to clients but could be read by someone with access to the local disk of a Redpanda node. 
:::