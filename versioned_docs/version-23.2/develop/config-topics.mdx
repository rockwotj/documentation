...
The DeleteRecords API lets you delete data from the beginning of a partition up to a specific business event (offset). The offset represents the true creation time of the event, not the time when it was stored by Redpanda. This frees up disk space, especially if your producers are pushing more data than you anticipated in your retention plan. 

Delete records from a topic with the `rpk topic trim` command. Use this when you know that all consumers have read up to that given offset, and the data is no longer needed.

:::note notes
- To delete records, `cleanup.policy` must be set to `delete` or `delete+compact`.
- Object storage is deleted asynchronously. When DeleteRecords returns, the partition's start offset will have advanced, but the work required to garbage collect deleted segments may not be complete.
- After calling DeleteRecords, local storage and object storage may still contain data for deleted offsets. (Redpanda does not truncate segments, instead it deletes as many whole segments as possible, then bumps the start offset.) Data before the new start offset is not visible to clients but could be read by someone with access to the local disk of a Redpanda node. 
:::