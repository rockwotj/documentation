...
The DeleteRecords API lets you delete data from the beginning of a partition up to a specific offset (event). The offset represents the true creation time of the event, not the time when it was stored by Redpanda. Calling this API frees up disk space, especially if your producers are pushing more data than you anticipated in your retention plan. 
