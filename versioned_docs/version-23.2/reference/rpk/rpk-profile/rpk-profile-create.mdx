...
If no `[NAME]` is provided, the name `default` is used.

There are multiple ways to create a profile:
...
- You can use `--from-cloud` to generate a profile from an existing cloud cluster
  ID. Note that you must be logged in with [`rpk cloud login`](../../rpk-cloud/rpk-cloud-login) first.
