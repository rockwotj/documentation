...
Default lifetime of log segments for topics which do not set `segment.ms` to override it. If `null`, the property is disabled, and no default lifetime is set. It's also accessible using the Kafka API under its Kafka-compatible alias, `log.roll.ms`.
...
This flag applies only for Redpanda clusters v23.1 or earlier that have upgraded to v23.2 or later. Newly created Redpanda clusters v23.2 or later will ignore this property. This flag will be removed in a future version of Redpanda, after which strings containing control characters won't be accepted.
