...
You can configure the environment variable `REDPANDA_BROKERS` for broker addresses so you don’t have to include the `-X brokers` flag each time you run an `rpk` command. For example, this is how you could configure three brokers on a single machine running on localhost:
